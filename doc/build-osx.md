...
====================================
This guide will show you how to build litedashd (headless client) for OSX.
...
After exiting, you'll get a warning that the install is keg-only, which means it wasn't symlinked to `/usr/local`.  You don't need it to link it to build litedash, but if you want to, here's how:
...
4.  (Optional) You can also install litedashd to your path:
...
4. Enter "litedash-qt" as project name, enter src/qt as location
...
It's now available at `./litedashd`, provided that you are still in the `src`
...
-------
    ...
    ./litedashd -daemon # to start the litedash daemon.
    ./litedash-cli --help  # for a list of command-line options.
    ./litedash-cli help    # When the daemon is running, to get a list of RPC commands
