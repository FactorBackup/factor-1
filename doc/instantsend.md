...
This mechanism has been integrated into Bitcore-Node-Flowercoin which allows for notification to be broadcast through Insight API in one of two ways:
...
By default, the Flowercoin Core daemon will launch using the following constant:

```
static const int DEFAULT_INSTANTSEND_DEPTH = 5;
```

This value can be overridden by passing the following argument to the Flowercoin Core daemon:
