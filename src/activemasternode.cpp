...
{    
    ...
    if(status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if(Params().NetworkID() == CBaseChainParams::MAIN) {
            if(service.GetPort() != 1993) {
                notCapableReason = strprintf("Invalid port: %u - only 1993 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if(service.GetPort() == 1993) {
            notCapableReason = strprintf("Invalid port: %u - 1993 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
...
bool CActiveMasternode::CreateBroadcast(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage, CMasternodeBroadcast &mnb, bool fOffline) {
    ...
    if(Params().NetworkID() == CBaseChainParams::MAIN) {
        if(service.GetPort() != 1993) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only 1993 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::CreateBroadcast() - %s\n", errorMessage);
            return false;
        }
    } else if(service.GetPort() == 1993) {
        errorMessage = strprintf("Invalid port %u for masternode %s - 1993 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::CreateBroadcast() - %s\n", errorMessage);
        return false;
    }
